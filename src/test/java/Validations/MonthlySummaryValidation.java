...
public class MonthlySummaryValidation {
	...
	public void validationRevenueInsertion() {
		...
		try {
			...
			Report.log(Status.PASS, "Teste Movimentação Receita Inserido Com Sucesso",
					Screenshot.captureBase64(driver));
		} catch (Exception e) {
			System.out.println(
					"MonthlySummaryValidation - validationRevenueInsertion - Ocorreu um erro, verificar log de erros.");
	...
	public void validationExpenseInsertion() {
		...
		try {
			...
			Report.log(Status.PASS, "Teste Movimentação Despesa Inserido Com Sucesso",
					Screenshot.captureBase64(driver));
		} catch (Exception e) {
			System.out.println(
					"MonthlySummaryValidation - validationExpenseInsertion - Ocorreu um erro, verificar log de erros.");
			Report.log(Status.FAIL, e.getMessage(), Screenshot.captureBase64(driver));
		}
